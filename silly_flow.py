...
def main():
    ...
    setup_logging(this_file.parent)

    my_arg = FlowArg(output_dir=this_file.parent, my_arg=42)

    log = get_logger(who="main")
    log.info("main: the deployment is set up now")
    log.debug("main: This is supposed to be in the logfile, but not the terminal or the prefect UI")
    ...
    flow_main.from_source(
        ...
        name="a-silly-flow",
        version="1.2.3",
        parameters = { "dataset": my_arg.model_dump() }
    )
